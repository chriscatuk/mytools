...
function get_ips_this_asg() {
    ...
    output=$(curl -s http://169.254.169.254/latest/dynamic/instance-identity/document)
    if [ "$?" -ne 0 -o -z "$output" ] ; then
        cjg_showerror "Can't recover Meta-Data. Aborting"; exit 1;
    fi

    read instanceID region <<< $(jq --compact-output --raw-output --exit-status '.instanceId, .region' 2> /dev/null <<< $output)
    if [ "$?" -ne 0 -o -z "$instanceID" -o -z "$region" ] ; then
        cjg_showerror "Can't extract Instance ID and Region from Meta-Data. Aborting"; exit 1;
    fi
