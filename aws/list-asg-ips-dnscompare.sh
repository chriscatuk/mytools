...
# compare them with all ip from a DNS A Record
...
cat << EOF  
    ...
    Usage: ./$0 -n <hostname> [-i <instance-id> -r <aws-region>] [--verbose] [--help] 
    (version: $version)
    ...
    -n <hostname>, --name <hostname>    DNS A Record to compare with. Ex: host1.example.com
...
function dns_resolve(){
    ...
    ips=$(dig +short $hostname -t A)

    if [ "$?" -ne 0 ] ; then
        cjg_logerror "DNS resolution error"
    fi

    if [ -z "$ips" ] ; then
        cjg_logdebug "DNS resolution couldn't find any IP"
    else
        cjg_logdebug "DNS resolution found the following IPs:"
        cjg_logdebug $ips
    fi
...
options=$(getopt -l "help,verbose,instanceid:,region,name:" -o "hvin:r:" -a -- "$@")
eval set -- "$options"
while  true
do
    case $1 in
...
if [ -z "$param_hostname" ] ; then
    cjg_logerror "You must specify the hostname to compare the IPs with, ex: -n host1.example.com"
    showHelp $0
    exit 2
fi
...
cjg_logdebug "Hostname provided: $param_hostname"
cjg_logdebug ""

dnsips=$(dns_resolve $param_hostname)
...
cjg_logdebug "IPs in DNS:"
cjg_logdebug "$dnsips"
cjg_logdebug "IPs in ASG"
cjg_logdebug "$ips"
