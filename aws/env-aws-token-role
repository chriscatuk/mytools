...
# Usage:
# source env-aws-token-role --serial-number arn:aws:iam::65AAAAAAAAAA869:mfa/firstname.lastname --role-arn arn:aws:iam::65AAAAAAAAAA869:role/RoleName --role-session-name RoleName  --duration-seconds 3600 --profile <mainprofile>
...
output=`aws sts assume-role --token-code $token --output json "$@"`
rc=$?

if [[ $rc -ne 0 ]] ; then
echo "Exiting with an error"
echo aws sts assume-role --token-code $token --output json "$@"
return $rc
fi
